...
import com.smartSense.feed.feedUI.FeedListFragment
...
class FeedListFragmentTest {
    ...
    fun is_RecyclerView_Visible_shouldBe_True() {
        onView(withId(R.id.rvFeedList)).check(matches(isDisplayed()))
    }

    @Test
    fun is_ListItem_Visible_shouldBe_True() {
        onView(withId(R.id.rvFeedList)).check(matches(hasDescendant(withId(R.id.feedThumbnailView))))
    }
